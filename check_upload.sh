#!/bin/bash
# Script to download / check and upload scikit-learn wheels for release
...
if [ "${SK_VERSION:0:1}" != 'v' ]; then
    echo "scikit learn version $SK_VERSION does not start with 'v'"
    exit 1
fi
WHEEL_HEAD="scikit_learn-${SK_VERSION:1}"
